...
export const Transfers = () => {
  ...
  const documents = useMemo(() => 
    allDocuments.filter(d => d.type === 'TRANSFER'), 
  [allDocuments]);
  ...
  const [dest, setDest] = useState('Distribution Center');

  const handleCreate = async () => {
    if (!ref) return;
    const id = await createDocument('TRANSFER', ref);
    setCurrentDocId(id);
    setIsCreating(true);
  };
      ...
      {isCreating ? (
        <div className="bg-brand-primary border border-white/10 rounded-xl p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="block text-sm text-slate-400 mb-2">Transfer Reference</label>
              <input 
                type="text" 
                value={ref}
            ...
            <div>
              <label className="block text-sm text-slate-400 mb-2">Source Location</label>
              <select 
                value={source} onChange={e => setSource(e.target.value)}
                className="bg-brand-dark border border-white/10 rounded-lg px-4 py-2 text-white w-full outline-none"
                disabled={!!currentDocId}
              >
            ...
            <div>
              <label className="block text-sm text-slate-400 mb-2">Destination Location</label>
              <select 
          ...
          {!currentDocId && (
             <button onClick={handleCreate} className="bg-brand-blue px-6 py-2 rounded-lg text-white w-full md:w-auto">Start Transfer</button>
          )}

          {currentDocId && (
            <>
              <div className="border-t border-white/10 pt-6">
                <h3 className="text-lg font-semibold text-white mb-4">Items to Move</h3>
              ...
              <div className="bg-brand-dark rounded-lg p-4">
                <p className="text-sm text-slate-400 mb-2">Transfer Manifest:</p>
                ...
                {currentItems.map((item, idx) => (
                  <div key={idx} className="flex justify-between py-2 border-b border-white/5 last:border-0">
                    ...
                    <div className="flex items-center gap-2 text-brand-blue font-mono">
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button 
                  ...
                  className="bg-brand-success hover:bg-emerald-600 text-white px-6 py-2 rounded-lg flex items-center gap-2 font-medium shadow-lg shadow-brand-success/20"
                >
                  <RefreshCw className="w-4 h-4" />
                  Complete Transfer
                </button>
        ...
        <div className="bg-brand-primary border border-white/5 rounded-xl overflow-hidden">
          <table className="w-full text-sm text-left text-slate-400">
            ...
            <tbody className="divide-y divide-white/5">
              {documents.length === 0 ? (
                 <tr><td colSpan={4} className="px-6 py-8 text-center text-slate-500">No transfers found.</td></tr>
              ) : (
                documents.map((doc) => (
