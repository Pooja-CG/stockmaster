...
export const Receipts = () => {
  ...
  const documents = useMemo(() => 
    allDocuments.filter(d => d.type === 'RECEIPT'), 
  [allDocuments]);
  ...
  const handleCreate = async () => {
    if (!ref) return;
    const id = await createDocument('RECEIPT', ref);
    if (id) {
      setCurrentDocId(id);
      setIsCreating(true);
    }
  ...
  const handleValidate = async () => {
    if (currentDocId) {
      await validateDocument(currentDocId);
      setIsCreating(false);
      setRef('');
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-white">Receipts (Incoming)</h1>
        {!isCreating && (
          <button 
      ...
      {isCreating ? (
        <div className="bg-brand-primary border border-white/10 rounded-xl p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <div className="grid grid-cols-2 gap-6">
            <div>
              <label className="block text-sm text-slate-400 mb-2">Reference / PO #</label>
          ...
          {currentDocId && (
            <>
              ...
              <div className="bg-brand-dark rounded-lg p-4">
                <p className="text-sm text-slate-400 mb-2">Items in this receipt:</p>
                ...
                {currentItems.map((item, idx) => (
                  <div key={idx} className="flex justify-between py-2 border-b border-white/5 last:border-0">
                    ...
                    <span className="text-brand-blue font-mono">+{item.quantity}</span>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-brand-primary border border-white/5 rounded-xl overflow-hidden">
          <table className="w-full text-sm text-left text-slate-400">
            ...
            <tbody className="divide-y divide-white/5">
              {documents.length === 0 ? (
                 <tr><td colSpan={4} className="px-6 py-8 text-center text-slate-500">No receipts found.</td></tr>
              ) : (
                documents.map((doc) => (
