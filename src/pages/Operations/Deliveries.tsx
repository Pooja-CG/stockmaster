...
export const Deliveries = () => {
  ...
  const documents = useMemo(() => 
    allDocuments.filter(d => d.type === 'DELIVERY'), 
  [allDocuments]);
  ...
  const handleCreate = async () => {
    if (!ref) return;
    const id = await createDocument('DELIVERY', ref);
    setCurrentDocId(id);
    setIsCreating(true);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-white">Deliveries (Outgoing)</h1>
        {!isCreating && (
          <button 
      ...
      {isCreating ? (
        <div className="bg-brand-primary border border-white/10 rounded-xl p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <div className="grid grid-cols-2 gap-6">
            <div>
              <label className="block text-sm text-slate-400 mb-2">Order Reference / SO #</label>
          ...
          {currentDocId && (
            <>
              <div className="border-t border-white/10 pt-6">
                <h3 className="text-lg font-semibold text-white mb-4">Pick Items</h3>
              ...
              <div className="bg-brand-dark rounded-lg p-4">
                <p className="text-sm text-slate-400 mb-2">Items to deliver:</p>
                ...
                {currentItems.map((item, idx) => (
                  <div key={idx} className="flex justify-between py-2 border-b border-white/5 last:border-0">
                    ...
                    <span className="text-brand-danger font-mono">-{item.quantity}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button 
                  ...
                  className="bg-brand-blue hover:bg-blue-600 text-white px-6 py-2 rounded-lg flex items-center gap-2 font-medium shadow-lg shadow-brand-blue/20"
                >
        ...
        <div className="bg-brand-primary border border-white/5 rounded-xl overflow-hidden">
          <table className="w-full text-sm text-left text-slate-400">
            ...
            <tbody className="divide-y divide-white/5">
              {documents.length === 0 ? (
                 <tr><td colSpan={4} className="px-6 py-8 text-center text-slate-500">No deliveries found.</td></tr>
              ) : (
                documents.map((doc) => (
